...
// Public portfolio link (created the first time the user opens Create Portfolio).
$portfolioSlug = '';
try {
    $stmt = $pdo->prepare("SELECT slug FROM portfolios WHERE user_id = ?");
    $stmt->execute([$userId]);
    $portfolioSlug = $stmt->fetchColumn() ?: '';
} catch (Throwable $e) {
    $portfolioSlug = ''; // portfolios table not imported yet
}
$viewPortfolioUrl = $portfolioSlug ? ('view-portfolio.php?u=' . urlencode($portfolioSlug)) : 'create-portfolio.php';
$viewPortfolioAttr = $portfolioSlug ? ' target="_blank" rel="noopener"' : '';
    ...
    <div class="app">
        ...
        <aside class="sidebar" id="sidebar">
            ...
            <nav class="nav">
                ...
                <a href="<?= htmlspecialchars($viewPortfolioUrl) ?>"<?= $viewPortfolioAttr ?>><span class="ic">👁</span> View Portfolio</a>
        ...
        <main class="main">
            ...
            <section class="hero">
                ...
                <a href="create-portfolio.php" class="cta" style="margin-left:10px;background:rgba(255,255,255,.18)">📁 Create Portfolio</a>
            </section>
            ...
            <section class="actions">
                <a href="create-portfolio.php" class="action-card">
                    <div class="ac-icon">📁</div>
                    <h4>Create Portfolio</h4>
                    <p>Add projects, images and links, then get a shareable URL.</p>
                    <span class="go">Go →</span>
                </a>
                <a href="<?= htmlspecialchars($viewPortfolioUrl) ?>"<?= $viewPortfolioAttr ?> class="action-card">
                    <div class="ac-icon">👁</div>
                    <h4>View Portfolio</h4>
                    <p><?= $portfolioSlug ? 'Open your live, public portfolio page.' : 'Create your portfolio first to get a public link.' ?></p>
                    <span class="go"><?= $portfolioSlug ? 'View ↗' : 'Start →' ?></span>
                </a>
