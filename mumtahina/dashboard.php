...
.card{background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);padding:24px;transition:.2s;box-shadow:var(--shadow)}
...
.pf-card .pf-foot a{color:var(--primary);font-weight:700}
.new-card{display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;
  border-style:dashed;color:var(--muted);min-height:190px;gap:6px;cursor:pointer}
.new-card:hover{color:var(--text);border-color:var(--accent)}
.new-card .plus{font-size:36px;line-height:1;color:var(--accent)}
.new-card span{font-size:13px}
.pf-cover{height:120px;border-radius:10px;background:var(--surface-2);overflow:hidden;display:grid;place-items:center;color:var(--muted);font-size:26px;margin-bottom:14px}
  ...
  <main class="main">
    ...
    <section class="grid">
      ...
      <div class="card pf-card">
        <div class="pf-cover">
          <?php if (!empty($pr['cover'])): ?><img src="<?= htmlspecialchars($pr['cover']) ?>" alt=""><?php else: ?>🖼<?php endif; ?>
        </div>
